...
async def start(update: Update, context: CallbackContext) -> None: await update.message.reply_text("Welcome to the Music Bot! Use /play <song_name> to start.")

async def play(update: Update, context: CallbackContext) -> None: if not context.args: await update.message.reply_text("Usage: /play <song name>") return
...
async def skip(update: Update, context: CallbackContext) -> None: next_song = queue.next_song() if next_song: await update.message.reply_text(f"Skipping... Now playing {next_song['name']}!") requests.post(f"{BACKEND_API_URL}/skip", json={"song_name": next_song['name'], "song_url": next_song['url']}) else: await update.message.reply_text("No more songs in the queue.")

async def stop(update: Update, context: CallbackContext) -> None: queue.clear() await update.message.reply_text("Stream ended.") requests.post(f"{BACKEND_API_URL}/stop")
